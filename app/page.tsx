...
export default function BuildCircle() {
  ...
  }, [page, channel, channelMessages, draft, attached, joined, registeredEvents, wishlistedEvents, eventMoved, activeCommunityName]);
...
function RegisterModal({ event, onRegister }: { event: RegisteredEvent; onRegister: () => void }) {
  ...
  const submit = (submission: FormEvent) => { submission.preventDefault(); setSubmitted(true); window.setTimeout(onRegister, 1800); };
  ...
  return <form className="register-modal" onSubmit={submit}><span className="pill live">{(event.type || "EVENT").toUpperCase()} · {event.location.includes("Online") ? "ONLINE" : "IN PERSON"}</span><h2>{event.title}</h2><p>{dateLabel} · {event.time}</p><div className="form-row"><label>NAME<input defaultValue="Arjun Nair" required/></label><label>EMAIL<input type="email" defaultValue="arjun@buildcircle.dev" required/></label></div><label>COLLEGE OR COMPANY<input placeholder="e.g. Independent builder"/></label><label>WHAT DO YOU WANT TO GET FROM THIS EVENT?<select defaultValue=""><option value="" disabled>Select one</option><option>Meet collaborators</option><option>Learn something practical</option><option>Share a work in progress</option></select></label><p className="form-note"><Icon name="lock" size={14}/> Your registration details are only shared with this event’s hosts.</p><button className="dark-button wide" type="submit">Confirm registration <Icon name="arrow" size={16}/></button></form>;
}

function PassModal({ event, registered, close }: { event: RegisteredEvent; registered: boolean; close: () => void }) {
