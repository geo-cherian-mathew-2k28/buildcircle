...
function RegisteredEventsCalendar({ events, onOpenEvent }: { events: RegisteredEvent[]; onOpenEvent: () => void }) {
  ...
  return <section className="registered-calendar" aria-label="Your registered event calendar"><div className="calendar-title"><span><Icon name="calendar" size={14}/><b>Your calendar</b></span><em>{events.length} registered</em></div><p>Registered events and reminders</p><div className="calendar-month"><b>{monthLabel}</b><span>IST</span></div><div className="calendar-weekdays">{"SMTWTFS".split("").map((day, index) => <span key={`${day}-${index}`}>{day}</span>)}</div><div className="calendar-grid">{Array.from({ length: startsOn }, (_, index) => <span className="calendar-empty" key={`blank-${index}`}/>) }{Array.from({ length: daysInMonth }, (_, index) => { const day = index + 1; const date = `${activeMonth}-${String(day).padStart(2, "0")}`; const hasEvent = events.some((event) => event.date === date); return <button key={date} className={`${selectedDate === date ? "selected" : ""} ${hasEvent ? "has-event" : ""}`} aria-pressed={selectedDate === date} onClick={() => selectDay(day)}>{day}</button>; })}</div><div className="calendar-agenda"><span>{dayLabel}</span>{selectedEvents.length ? selectedEvents.map((event) => <button key={event.id} onClick={onOpenEvent}><i/><div><b>{event.title}</b><small>{event.time}</small></div><Icon name="chevron" size={13}/></button>) : <p>{events.length ? "No registered events on this day." : "Register for an event to add it here."}</p>}</div>{events.length > 0 && <div className="calendar-reminder"><Icon name="bell" size={13}/><span>24-hour and 1-hour reminders are scheduled.</span></div>}</section>;
}
...
export default function BuildCircle() {
  ...
  const registered = registeredEvents.some((event) => event.id === featuredRegisteredEvent.id);
  ...
  const registerForFeaturedEvent = () => {
    ...
    setWishlistedEvents((events) => events.filter((event) => event.id !== featuredRegisteredEvent.id));
    setModal("pass");
  ...
  const content = useMemo(() => {
    ...
    if (page === "events") return <EventsView registered={registered} eventMoved={eventMoved} wishlistedEvents={wishlistedEvents} onRegister={() => setModal("register")} onPass={() => setModal("pass")} onCheckin={() => setModal("checkin")} onImpact={() => setModal("impact")} onMove={() => { setEventMoved(true); notify("Event time updated — attendees are being reviewed"); }} onToggleWishlist={toggleWishlist} />;
    if (page === "calendar") return <CalendarView registeredEvents={registeredEvents} wishlistedEvents={wishlistedEvents} onOpenEvents={() => setPage("events")} onToggleWishlist={toggleWishlist} />;
    if (page === "notifications") return <NotificationsView onOpen={() => setPage("community")} />;
    if (page === "profile") return <ProfileView />;
    return <HomeView onExplore={() => setPage("explore")} onCommunity={() => { setActiveCommunityName("AI Agents"); setPage("community"); }} onEvents={() => setPage("events")} onDigest={() => setModal("digest")} />;
  }, [page, channel, channelMessages, draft, attached, joined, registered, registeredEvents, wishlistedEvents, eventMoved, activeCommunityName]);
  ...
  return (
    <main className="app-shell">
      <aside className="sidebar">
        ...
        <RegisteredEventsCalendar events={registeredEvents} onOpenEvent={() => switchPage("calendar")} />
...
function CommunityView({ communityName, channel, setChannel, channelMessages, setChannelMessages, draft, setDraft, attached, setAttached, attachmentName, setAttachmentName, attachedAudio, setAttachedAudio, onSubmit, onShowcase, onMembership, onSwitchCommunity, onBrowseCommunities }: { communityName: string; channel: string; setChannel: (value: string) => void; channelMessages: Record<string, Message[]>; setChannelMessages: React.Dispatch<React.SetStateAction<Record<string, Message[]>>>; draft: string; setDraft: (value: string) => void; attached: string | null; setAttached: (value: string | null) => void; attachmentName: string; setAttachmentName: (value: string) => void; attachedAudio: string | null; setAttachedAudio: (value: string | null) => void; onSubmit: (event: FormEvent) => void; onShowcase: () => void; onMembership: () => void; onSwitchCommunity: (name: string) => void; onBrowseCommunities: () => void }) {
...
function CalendarView({ registeredEvents, wishlistedEvents, onOpenEvents, onToggleWishlist }: { registeredEvents: RegisteredEvent[]; wishlistedEvents: RegisteredEvent[]; onOpenEvents: () => void; onToggleWishlist: (event: RegisteredEvent) => void }) {
...
function CreateModal({ close, onCreate }: { close: () => void; onCreate: (draft: { name: string; description: string; category: string; access: "public" | "approval" }) => void }) {
  ...
  return <form className="create-modal" onSubmit={submit}><p className="eyebrow">START SOMETHING MEANINGFUL</p><h2>Create a {mode}</h2><div className="segmented"><button type="button" className={mode === "community" ? "active" : ""} onClick={() => setMode("community")}><Icon name="people" size={17}/> Community</button><button type="button" className={mode === "event" ? "active" : ""} onClick={() => setMode("event")}><Icon name="calendar" size={17}/> Event</button></div><label>NAME<input name="name" placeholder={mode === "community" ? "e.g. Creative Coders Chennai" : "e.g. Build night #1"} required/></label><label>{mode === "community" ? "WHAT WILL PEOPLE BUILD OR LEARN HERE?" : "A SHORT DESCRIPTION"}<textarea name="description" placeholder={mode === "community" ? "Give your future members a clear, inviting reason to join." : "What’s special about this gathering?"} required/></label>{mode === "community" ? <><label>CATEGORY<select name="category" defaultValue="" required><option value="" disabled>Choose a category</option><option>AI</option><option>Software</option><option>Design</option><option>Hardware</option><option>Open source</option><option>Startups</option></select></label><label>JOIN MODE<div className="radio-options"><button type="button" className={access === "public" ? "selected" : ""} onClick={() => setAccess("public")}><span>{access === "public" ? "●" : "○"}</span><div><b>Public</b><small>Anyone can join instantly</small></div></button><button type="button" className={access === "approval" ? "selected" : ""} onClick={() => setAccess("approval")}><span>{access === "approval" ? "●" : "○"}</span><div><b>Approval required</b><small>Review every request</small></div></button></div></label><div className="channel-blueprint"><b>YOUR STARTER CHANNELS</b><span># general</span><span># introductions</span><span># projects</span><span># announcements</span><small>You can add, edit, or restrict channels from community settings.</small></div></> : <label>WHEN<input type="datetime-local"/></label>}<div className="profile-dialog-actions"><button className="outline-button" type="button" onClick={close}>Cancel</button><button className="dark-button" type="submit">Create {mode} <Icon name="arrow" size={16}/></button></div></form>;
}
