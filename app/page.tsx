...
function CommunityView({ profile, communityName, channel, setChannel, channelMessages, setChannelMessages, draft, setDraft, attached, setAttached, attachmentName, setAttachmentName, attachedAudio, setAttachedAudio, onSubmit, onShowcase, onMembership, onSwitchCommunity, onBrowseCommunities }: { profile: UserProfile; communityName: string; channel: string; setChannel: (value: string) => void; channelMessages: Record<string, Message[]>; setChannelMessages: React.Dispatch<React.SetStateAction<Record<string, Message[]>>>; draft: string; setDraft: (value: string) => void; attached: string | null; setAttached: (value: string | null) => void; attachmentName: string; setAttachmentName: (value: string) => void; attachedAudio: string | null; setAttachedAudio: (value: string | null) => void; onSubmit: (event: FormEvent) => void; onShowcase: () => void; onMembership: () => void; onSwitchCommunity: (name: string) => void; onBrowseCommunities: () => void }) {
  ...
  const [workspacePanel, setWorkspacePanel] = useState<"none" | "messages" | "pulse">("none");
  ...
  const [channelUnreadCounts, setChannelUnreadCounts] = useState<Record<string, number>>({ "AI Agents:help": 4, "AI Agents:projects": 2, "Codex Builders:general": 3, "IoT Builders:help": 3, "Open Source Kerala:announcements": 2 });
  const [channelReadReady, setChannelReadReady] = useState(false);
  const [sprintJoined, setSprintJoined] = useState(false);
  const [weeklyGoalClaimed, setWeeklyGoalClaimed] = useState(false);
  ...
  useEffect(() => {
    try {
      const storedUnread = window.localStorage.getItem("buildcircle-channel-unread");
      if (storedUnread) {
        const parsedUnread = JSON.parse(storedUnread) as unknown;
        if (parsedUnread && typeof parsedUnread === "object" && !Array.isArray(parsedUnread)) setChannelUnreadCounts(parsedUnread as Record<string, number>);
      }
    } catch {
      window.localStorage.removeItem("buildcircle-channel-unread");
    } finally {
      setChannelReadReady(true);
    }
  }, []);
  useEffect(() => {
    if (channelReadReady) window.localStorage.setItem("buildcircle-channel-unread", JSON.stringify(channelUnreadCounts));
  }, [channelReadReady, channelUnreadCounts]);
  ...
  const communityMemberTotal = currentCommunity?.members || "0";
  const nextCommunityEvent = upcomingEvents.find((event) => event.community === communityName) || featuredRegisteredEvent;
  ...
  const unreadFor = (id: string, name = communityName) => channelUnreadCounts[`${name}:${id}`] || 0;
  const channelUnreadCount = unreadFor(channel);
  const markChannelRead = (id = channel, name = communityName) => setChannelUnreadCounts((current) => current[`${name}:${id}`] ? { ...current, [`${name}:${id}`]: 0 } : current);
  const switchChannel = (next: string) => { markChannelRead(next); setChannel(next); setThread(null); setThreadDraft(""); setDraft(""); setAttached(null); };
  ...
  return <div className="community-layout">
    ...
    {thread && selected && <aside className="thread-panel"><header><button onClick={() => setThread(null)}><Icon name="close" size={18}/></button><div><b>Thread</b><small>#{channel}</small></div></header><div className="thread-root"><b>{selected.user}</b><p>{selected.body}</p></div><div className="thread-count">{selectedReplies.length} {selectedReplies.length === 1 ? "reply" : "replies"}</div>{selectedReplies.length ? selectedReplies.map((reply) => <div className="thread-reply" key={reply.id}>{avatar(reply.user, reply.shade)}<div><b>{reply.user}</b><p>{reply.body}</p><small>{reply.time}</small></div></div>) : <p className="empty-thread">No replies yet. Be the first to add a useful thought.</p>}<form className="thread-composer" onSubmit={sendThreadReply}><input value={threadDraft} onChange={(event) => setThreadDraft(event.target.value)} placeholder={`Reply to ${selected.user}`}/><button type="submit" aria-label="Send thread reply"><Icon name="send" size={16}/></button></form></aside>}
        {!thread && workspacePanel === "pulse" && <aside className="community-pulse-panel"><header><button onClick={() => setWorkspacePanel("none")} aria-label="Close community pulse"><Icon name="close" size={18}/></button><div><b>Community Pulse</b><small>{communityName} · this week</small></div></header><div className="pulse-body"><section className="pulse-hero"><span><Icon name="sparkle" size={18}/></span><div><p>BUILD WITH INTENT</p><h3>Small progress, shared momentum.</h3><small>{sprintJoined ? "You’re in the current focus sprint." : "Join other builders for one uninterrupted block."}</small></div></section><div className="pulse-stats"><div><b>{sprintJoined ? "18" : "17"}</b><span>in focus now</span></div><div><b>{buildTokens}</b><span>BuildTokens</span></div><div><b>{weeklyGoalClaimed ? "1/1" : "0/1"}</b><span>weekly goal</span></div></div><section className="pulse-card"><div className="pulse-card-head"><span className="pulse-kicker"><i/> LIVE SPRINT</span><small>28 min left</small></div><b>Ship one useful improvement</b><p>Share a question, a build note, or feedback that unblocks someone.</p><button className={sprintJoined ? "pulse-joined" : "pulse-cta"} onClick={() => { setSprintJoined((joined) => !joined); setBuildTokens((balance) => sprintJoined ? Math.max(0, balance - 5) : balance + 5); }}>{sprintJoined ? <><Icon name="check" size={14}/> You joined · +5 tokens</> : <><Icon name="sparkle" size={14}/> Join focus sprint</>}</button></section><section className="pulse-card compact"><div className="pulse-card-head"><span className="pulse-kicker">WEEKLY QUEST</span><small>{weeklyGoalClaimed ? "Complete" : "Optional"}</small></div><b>Make one relevant introduction</b><p>Connect two builders who could genuinely help each other.</p><button className="pulse-text-action" onClick={() => setWeeklyGoalClaimed((claimed) => !claimed)}>{weeklyGoalClaimed ? "Undo completion" : "Mark complete"} <Icon name={weeklyGoalClaimed ? "check" : "arrow"} size={14}/></button></section><section className="pulse-event"><span><Icon name="calendar" size={17}/></span><div><p>UP NEXT</p><b>{nextCommunityEvent.title}</b><small>{nextCommunityEvent.time} · {nextCommunityEvent.location}</small></div></section></div></aside>}{!thread && workspacePanel === "messages" && <aside className="direct-panel"><header><button onClick={() => setWorkspacePanel("none")} aria-label="Close direct messages"><Icon name="close" size={18}/></button><div><b>Messages</b><small>Private conversations</small></div></header><div className="direct-contact-list">{directContacts.map(([name, shade, status]) => <button className={activeChat === name ? "active" : ""} key={name} onClick={() => setActiveChat(name)}>{avatar(name, shade)}<span><b>{name}</b><small>{status}</small></span><i/></button>)}</div><div className="direct-conversation"><p>Private and only visible to this conversation.</p>{(directMessages[activeChat] || []).map((message, index) => <div className={message.startsWith("You:") ? "from-you" : ""} key={`${message}-${index}`}><b>{message.startsWith("You:") ? "You" : activeChat}</b><span>{message.replace(/^You: /, "")}</span></div>)}</div><form className="direct-composer" onSubmit={sendDirectMessage}><input value={chatDraft} onChange={(event) => setChatDraft(event.target.value)} placeholder={`Message ${activeChat}`}/><button type="submit" aria-label="Send direct message"><Icon name="send" size={16}/></button></form></aside>}
  </div>;}
...
function ModalLayer({ type, close, profile, communityName, registered, registrationEvent, checkedIn, eventMoved, onNavigate, onRegister, onCheckin, onMove, onShowcase, onCreate }: { type: Modal; close: () => void; profile: UserProfile; communityName: string; registered: boolean; registrationEvent: RegisteredEvent; checkedIn: boolean; eventMoved: boolean; onNavigate: (target: DigestDestination) => void; onRegister: () => void; onCheckin: () => void; onMove: () => void; onShowcase: (project: { name: string; description: string; image?: string }) => void; onCreate: (draft: { name: string; description: string; category: string; access: "public" | "approval" }) => void }) {
  return <div className="modal-backdrop" role="dialog" aria-modal="true" onMouseDown={close}><section className={`modal ${type === "digest" || type === "impact" ? "modal-wide" : ""}`} onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={close}><Icon name="close" size={19}/></button>{type === "create" && <CreateModal close={close} onCreate={onCreate}/>} {type === "register" && <RegisterModal event={registrationEvent} profile={profile} onRegister={onRegister}/>} {type === "pass" && <PassModal event={registrationEvent} profile={profile} registered={registered} close={close}/>} {type === "checkin" && <CheckinModal profile={profile} checkedIn={checkedIn} onCheckin={onCheckin}/>} {type === "digest" && <DigestModal close={close} onNavigate={onNavigate}/>} {type === "impact" && <ImpactModal eventMoved={eventMoved} onMove={onMove} close={close}/>} {type === "showcase" && <ShowcaseModal publish={onShowcase}/>} {type === "membership" && <MembershipModal profile={profile} communityName={communityName} close={close}/>}</section></div>;
}

function MembershipModal({ profile, communityName, close }: { profile: UserProfile; communityName: string; close: () => void }) {
  const [query, setQuery] = useState("");
  const community = communities.find((item) => item.name === communityName);
  const memberTotal = community?.members || "0";
  const members = [
  ...
  return <div className="membership-modal member-directory"><div className="membership-hero"><MiniLogo color="teal"/><div><p className="eyebrow">{communityName.toUpperCase()}</p><h2>Community members</h2><p>{memberTotal} builders learning and shipping together</p></div></div><label className="member-search"><Icon name="search" size={16}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a member"/></label><div className="member-directory-meta"><span>{query ? `${visible.length} matches` : "People active this week"}</span><span>Role</span></div><div className="member-list">{visible.map((member) => <div className="member-row" key={member.name}>{avatar(member.name, member.shade)}<div><b>{member.name}</b><small>{member.note}</small></div><span className={`role-badge ${member.role.toLowerCase().replace(" ", "-")}`}>{member.role}</span></div>)}{visible.length === 0 && <p className="member-empty">No members match “{query}”.</p>}</div><div className="membership-footer"><span>Showing active community members</span><button className="outline-button" onClick={close}>Done</button></div></div>;
}
